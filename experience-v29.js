(() => {
    ...
    const featureObs=new IntersectionObserver(entries=>{
      entries.forEach(entry=>{
        if(entry.isIntersecting && entry.intersectionRatio>.48){
          ...
          if(!aokoVoicePlayed) setTimeout(()=>playAokoVoice(),420);
        }
      });
    },{threshold:[.25,.48,.72]});
  ...
  const galleryItems=qa('.gallery-piece[data-gallery-src]');
  const exhibition=q('#gallery-exhibition'),exhibitionImg=q('#gallery-exhibition-image'),exhibitionTitle=q('#gallery-exhibition-title');
  let galleryIndex=0;
  function openGallery(index){
    ...
    if(exhibitionImg){exhibitionImg.src=src;exhibitionImg.alt=item.dataset.galleryTitle||'Head in the Clouds artwork';}
    if(exhibitionTitle) exhibitionTitle.textContent=item.dataset.galleryTitle||'Artwork';
    exhibition.hidden=false;requestAnimationFrame(()=>exhibition.classList.add('is-open'));
    body.classList.add('modal-open');
    const viewed=new Set(loadJSON('hitc-gallery-viewed-v29',[]));viewed.add(src);save('hitc-gallery-viewed-v29',[...viewed]);item.dataset.viewed='true';
  }
  function closeGallery(){if(!exhibition)return;exhibition.classList.remove('is-open');body.classList.remove('modal-open');setTimeout(()=>{if(!exhibition.classList.contains('is-open'))exhibition.hidden=true;},420);}
  galleryItems.forEach((item,i)=>{
    item.setAttribute('role','button');item.setAttribute('tabindex','0');item.setAttribute('aria-label',`Open ${item.dataset.galleryTitle||'artwork'} in exhibition view`);
  ...
  const viewedGallery=new Set(loadJSON('hitc-gallery-viewed-v29',[]));galleryItems.forEach(i=>{if(viewedGallery.has(i.dataset.gallerySrc))i.dataset.viewed='true';});
  ...
  // Trailer mode: a 40-second auto-directed pass through the website.
  const trailerButton=q('#hisano-trailer-start'),trailerOverlay=q('#trailer-overlay'),trailerProgress=q('#trailer-progress');
  let trailerActive=false,trailerTimers=[],trailerRAF=0,trailerStarted=0;
  const trailerStops=['top','story','characters','aoko-feature','hisano','history','scenes','gallery','preview','closing-sequence'];
  function clearTrailerTimers(){trailerTimers.forEach(clearTimeout);trailerTimers=[];cancelAnimationFrame(trailerRAF);}
  function stopTrailer(){
    if(!trailerActive)return;trailerActive=false;clearTrailerTimers();body.classList.remove('trailer-mode');
  ...
  function trailerFrame(){
    ...
    const p=Math.min(1,(performance.now()-trailerStarted)/40000);
    if(trailerProgress)trailerProgress.style.width=`${p*100}%`;
    trailerRAF=requestAnimationFrame(trailerFrame);
  }
  function startTrailer(){
    if(trailerActive)return;trailerActive=true;body.classList.add('trailer-mode');if(trailerOverlay)trailerOverlay.hidden=false;
    trailerStarted=performance.now();trailerFrame();
    trailerStops.forEach((id,i)=>{
      trailerTimers.push(setTimeout(()=>q(`#${id}`)?.scrollIntoView({behavior:isReduced()?'auto':'smooth',block:'start'}),i*4100));
    });
    trailerTimers.push(setTimeout(stopTrailer,40500));
    setTimeout(()=>{
      window.addEventListener('wheel',stopTrailer,true);window.addEventListener('pointerdown',stopTrailer,true);
      window.addEventListener('keydown',stopTrailer,true);
    },300);
  }
