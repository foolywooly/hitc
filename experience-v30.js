(() => {
  ...
  // Trailer mode: a visible, 40-second auto-directed cut through the core novel site.
  ...
  const trailerStops=[
    ...
    {id:'characters',at:5600,sound:'characters'},
    {id:'hisano',at:11500,sound:'hisano'},
    {id:'history',at:17800,sound:'history'},
    {id:'preview',at:24600,sound:'preview'},
    {id:'closing-sequence',at:32700,sound:'home'}
  ];
  ...
  function stopTrailer(){
    ...
    if(trailerOverlay)trailerOverlay.hidden=true;if(trailerPrelude)trailerPrelude.hidden=true;
  ...
  function beginTrailerCuts(){
    if(!trailerActive)return;
    if(trailerPrelude)trailerPrelude.hidden=true;
    ...
    trailerStops.forEach(stop=>{
      trailerTimers.push(setTimeout(()=>{
        ...
        audioApi.playUi?.(stop.sound||'default',q(`#${stop.id}`));
        body.dataset.trailerScene=stop.id;
        if(stop.id==='hisano'){const map=q('.map-slot');map?.classList.add('trailer-map-focus');setTimeout(()=>map?.classList.remove('trailer-map-focus'),1200);}
      },stop.at));
    });
    trailerTimers.push(setTimeout(stopTrailer,40000));
    setTimeout(()=>{
      window.addEventListener('wheel',stopTrailer,true);window.addEventListener('pointerdown',stopTrailer,true);window.addEventListener('keydown',stopTrailer,true);
    },350);
  }
  async function startTrailer(){
    ...
    if(trailerPrelude)trailerPrelude.hidden=false;
    const minimum=new Promise(r=>setTimeout(r,900));
    await Promise.all([preloadTrailerArt(),minimum]);
    beginTrailerCuts();
  }
